...
- real person prepare using MediaPipe + rembg/MODNet
...
def prepare(base_url: str, image_path: Path, label: str) -> Tuple[dict, float]:
    ...
    if debug.get("mattingEngine") not in {"rembg", "modnet"}:
        raise VerifyError(f"{label} did not use rembg/MODNet: {debug}")
...
def run_person_case(base_url: str, sample_id: str, gender: str, image_path: Path, source_url: str) -> SampleResult:
    ...
    try:
        ...
        result.rembg_model = debug.get("rembgModel", "")
...
def write_final_reports(
    ...
    md = [
        ...
        "- rembg/MODNet: verified by prepare debug `mattingEngine=rembg|modnet`",
        "- OpenCV/Pillow: used by backend matting postprocess, composition, and report image checks",
        "- MODNet: optional reserved interface; current fallback is rembg when weights are unavailable",
        "",
