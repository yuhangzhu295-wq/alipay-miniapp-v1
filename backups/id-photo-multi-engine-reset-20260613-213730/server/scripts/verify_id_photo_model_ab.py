...
REPORT_DIR = ROOT / "reports" / "id-photo-open-source-audit" / "model-ab-test"
...
def main() -> int:
    ...
    payload = {
        ...
        "decision": "Keep current rembg/u2net_human_seg production path with ID-photo-specific post-processing; do not switch to optional models without separate deployment and license review.",
    }
    write_json(REPORT_DIR / "model-ab-test-report.json", payload)
    lines = [
        ...
        "- Production decision: keep current rembg/u2net_human_seg plus local post-processing.",
        "",
        "## Tested Rows",
