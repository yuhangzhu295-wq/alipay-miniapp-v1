/**
 * Shared client-side coordinator for the server-owned WeChat image-safety Gate.
...
function _ensureOpenIdIdentity() {
  var auth = authService.getAuth();
  if (auth && auth.openidBound === true) return Promise.resolve(auth);
  return authService.loginWithProfile(authService.getUserInfo() || {}).then(function(nextAuth) {
    if (nextAuth && nextAuth.openidBound === true) return nextAuth;
    throw _makeSafetyError('CONTENT_SAFETY_OPENID_REQUIRED', UNAVAILABLE_MESSAGE, 'ERROR');
  }).catch(function(err) {
...
function ensureImageSafety(imagePath, purpose) {
  ...
  return _ensureOpenIdIdentity().then(function(auth) {
